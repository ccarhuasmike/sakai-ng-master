...
export class ExcelService {
    ...
    generateExcel(header:any, excelName:any, sheetName:any, isCurrency:any, data:any, dates:any, filterLabel:any) {
        ...
        data.forEach((d:any) => {
            ...
            // para convertir el valor en decimal solo funciona con valores numeros que te trae la api, aqui recorre celda por celda
            // row.getCell(3).numFmt = '#,##0.00';
            // row.getCell(4).numFmt = '#,##0.00';

            isCurrency.forEach((element:any) => {
                let cellValue = row.getCell(element).value;
                row.getCell(element).value = cellValue ? cellValue : 0;
